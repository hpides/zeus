...
public class Queries {
    ...
    public static Query makeFilter0Measured(Source<Tuple2<Integer, Long>> source, Sink<Tuple> sink) {
        return new VulcanoTopologyBuilder().streamOf(source).map(t -> t.v1).filter(e -> e % 2 == 0).flatProfiling()
    ...
    public static <In, Other> Join<Tuple3<In, Long, Long>, Tuple3<Other, Long, Long>, Tuple3<Tuple2<In, Other>, Long, Long>> makeJoinF() {
        return (Tuple3<In, Long, Long> t1, Tuple3<Other, Long, Long> t2) -> new Tuple3<>(new Tuple2<>(t1.v1, t2.v1),
    ...
    public static Query makeNexmarkLightPlainJoinMeasured(
        ...
        var j1 = s1.window(TumblingWindow.ofEventTime(Time.seconds(5))).join(s2, makeJoinF(), t1 -> t1.v1.v2,
                t2 -> t2.v1.v1, WatermarkGenerator.seconds(0, 1_000), TimestampExtractor.currentTimeNS());
        return j1.map(Queries::setEjectTimestamp).to(sink).buildAsQuery();
    }
    ...
    public static Query makeNexmarkLightAJoinCapacityMeasured(
        ...
        return j1.map(Queries::setEjectTimestamp).window(TumblingWindow.ofEventTime(Time.seconds(5))).aggregate(
                new Aggregator<Tuple3<Long, Long, Long>, Tuple4<Integer, Double, Double, Double>, Tuple4<Integer, Double, Double, Double>>() {
                    ...
                    @Override
                    public Tuple4<Integer, Double, Double, Double> getResult(
                            Tuple4<Integer, Double, Double, Double> state) {
                        return state;
                    }
    ...
    public static Query makeNexmarkHottestCategory(
        ...
        var s2 = tp.streamOf(auctionSource2).map(Queries::prepare);
        var j1 = s1.window(TumblingWindow.ofEventTime(Time.seconds(5))).ajoin(s2, t1 -> t1.v1.v2, t2 -> t2.v1.v1,
                makeJoinF(), e -> System.nanoTime(), new WatermarkGenerator<>(0, 10), "ajoinHottestCategory");
        return j1.window(TumblingWindow.ofEventTime(Time.seconds(5))).groupBy(t1 -> t1.v1.v2.v3).aggregate(
                new Aggregator<Tuple3<Tuple2<Tuple4<Long, Long, Integer, Integer>, Tuple4<Long, Integer, Integer, Integer>>, Long, Long>, Tuple3<Integer, Long, Long>, Tuple3<Integer, Long, Long>>() {
                ...
                .window(TumblingWindow.ofEventTime(Time.seconds(5))).aggregate(
                        new Aggregator<Tuple3<Integer, Long, Long>, Tuple3<Integer, Long, Long>, Tuple3<Integer, Long, Long>>() {
    ...
    public static Query makeNexmarkMaxiumPriceForAuction(
        ...
        var s2 = tp.streamOf(auctionSource2).map(Queries::prepare);
        var j1 = s1.window(TumblingWindow.ofEventTime(Time.seconds(5))).ajoin(s2, t1 -> t1.v1.v2, t2 -> t2.v1.v1,
                makeJoinF(), e -> System.nanoTime(), new WatermarkGenerator<>(0, 10), "ajoinMaximumPrice");
        return j1.filter(t2 -> t2.v1.v1.v4 > t2.v1.v2.v4).window(TumblingWindow.ofEventTime(Time.seconds(5)))
                .groupBy(t1 -> t1.v1.v2.v1).aggregate(
                        new Aggregator<Tuple3<Tuple2<Tuple4<Long, Long, Integer, Integer>, Tuple4<Long, Integer, Integer, Integer>>, Long, Long>, Tuple3<Integer, Long, Long>, Tuple3<Integer, Long, Long>>() {
                            ...
                            public Tuple3<Integer, Long, Long> add(Tuple3<Integer, Long, Long> state,
                                ...
                                if (input.v1.v1.v4 > state.v1) {
                                    return new Tuple3<>(input.v1.v1.v4, Math.max(state.v2, input.v2),
                                            Math.max(state.v3, input.v3));
                                } else {
                                    return new Tuple3<>(state.v1, Math.max(state.v2, input.v2),
                                            Math.max(state.v3, input.v3));
                                }
                            }
