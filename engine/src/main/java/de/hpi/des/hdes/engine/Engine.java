...
public class Engine {
  ...
  public synchronized void addQuery(Topology topology) {
      if(!isInitialized) {
          ...
          for (Node newSource : topology.getSourceNodes()) {
          ...
          ExecutionPlan extendPlan = ExecutionPlan.extend(topology, nodeIdToSourceSlotMap);
          ...
          for (Slot slot : extendPlan.getSlots()) {
              if (!slot.isAlreadyRunning()) {
                  ...
                  this.currentTopology.addNode(topology.getNodeById(slot.getTopologyNodeId()));
              }
          }
      }
  }
