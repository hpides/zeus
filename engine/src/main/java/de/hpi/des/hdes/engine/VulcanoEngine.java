 ...
 *
 * It is either running or not running. In both states it is possible to add or
 * remove queries.
 */
@Slf4j
public class VulcanoEngine {
   ...
   *
   * Note: We synchronize this method to avoid problems when multiple queries are
   * added or deleted at the same time
   *
  ...
  public synchronized void deleteQuery(final Query query) {
    // We synchronize this method to avoid problems when multiple queries are added
    // or
